...
class TenantScopedRepository(BaseRepository[ModelT]):
    ...
    def _scope(self, stmt: Select) -> Select:
        # `self.model.tenant_id` is a SQLAlchemy declarative attribute, which
        # mypy cannot see through an unbound TypeVar. Every model reaching here
        # carries `TenantOwnedMixin`, so the attribute always exists.
    ...
    def add(self, entity: ModelT) -> ModelT:
        entity_tenant_id = entity.tenant_id  # type: ignore[attr-defined]
